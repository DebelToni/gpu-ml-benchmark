// ai_bench.cu — uniform prints, fallback chain: FP4(native?)->INT8->FP4(emul) +
// FP16 baseline Build: nvcc -O3 -std=c++17 ai_bench.cu -lcublasLt -lcublas -o
// ai_bench
...
static inline void timer_begin(cudaEvent_t &a, cudaEvent_t &b) {
  CHECK_CUDA(cudaEventCreate(&a));
  CHECK_CUDA(cudaEventCreate(&b));
  CHECK_CUDA(cudaEventRecord(a));
}
static inline float timer_end(cudaEvent_t a, cudaEvent_t b, int iters) {
...
static GemmRes bench_int8_lt(int M, int N, int K, int iters, int *ok,
  ...
  *ok = 0;
  ...
  double ops = 2.0 * (double)M * N * K;
  r.ms = ms;
  r.tflops = (float)(ops / ((ms / 1000.0) * 1e12));
  cublasLtDestroy(lt);
  cudaFree(Ah);
  cudaFree(Bh);
  cudaFree(A8);
...
// ---------------- inference estimates ----------------
static void print_infer_estimates(double tflops_mix, double gbps,
                                  const char *low_name) {
  // params
  ...
  if (low_name && low_name[0] == 'i')
    s_w_low = 1.0; // int8

  auto toks = [&](int t, double s_w) -> double {
    const double F_layer =
        (8.0 + 4.0 * r) * (double)d * (double)d + 4.0 * t * (double)d;
  ...
  for (int i = 0; i < 3; i++) {
    ...
    const char *label =
        (low_name && low_name[0] == 'i') ? "INT8-weights" : "FP4-weights";
...
int main() {
  ...
  printf("Budgets: FP16 %.1fs, FP4 %.1fs, KV %.1fs\n", BUDGET_FP16_S,
         BUDGET_FP4_S, BUDGET_KV_S);

  // FP16
  GemmRes r16 = bench_fp16_tc(M, N, K, 10);
  printf("FP16 TC GEMM: %.2f TFLOPS (avg %.2f ms)\n", r16.tflops, r16.ms);

  // Low-precision chain: try INT8, else FP4 emu. (Native FP4 gated by future
  // toolchains.)
  ...
  // KV bandwidth
  float gbps = 0, kv_ms = 0;
  bench_kv(5.0, &gbps, &kv_ms);
  if (std::isnan(kv_ms))
    printf("KV bandwidth: skipped (insufficient memory)\n");
  else
  ...
  if (low_name[0] == 'i')
    printf("Selected low-precision mode: INT8\n");
  else
    printf("Selected low-precision mode: FP4(emulated)\n");
  printf("Selected low-precision fraction: %.2f\n", frac_low);
  printf("Mixed GEMM throughput: %.2f TFLOPS\n", mix_tflops);
  printf("Dynamic weight p=%.3f (higher compute -> lower p -> memory weighs "
         "more)\n",
         pwt);
