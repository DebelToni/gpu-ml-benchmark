// ai_bench.cu — fallback chain: FP4(native if available) -> INT8 -> FP4(emulated) + FP16 baseline
...
static inline float time_ms_begin(cudaEvent_t& a, cudaEvent_t& b){ CHECK_CUDA(cudaEventCreate(&a)); CHECK_CUDA(cudaEventCreate(&b)); return 0.f; }
static inline float time_ms_end(cudaEvent_t a, cudaEvent_t b, int iters){ CHECK_CUDA(cudaEventRecord(b)); CHECK_CUDA(cudaEventSynchronize(b)); float ms=0; CHECK_CUDA(cudaEventElapsedTime(&ms,a,b)); CHECK_CUDA(cudaEventDestroy(a)); CHECK_CUDA(cudaEventDestroy(b)); return ms/iters; }
...
static GemmRes bench_fp16_tc(int M,int N,int K,int iters){
  ...
  cudaEvent_t s,e; time_ms_begin(s,e);
...
static float bench_kv(double seconds, float* GBps){
  ...
  for(int i=0;i<reps;i++){ kv_stream<<<gs,bs>>>(K,V,O,n_vec); }
...
int main(){
  ...
  // TODO: implement cuBLASLt NVFP4 when toolchain exposes full API on your system.
  // For now treat as unavailable to avoid build breaks.
  ok = 0;
#else
  ok = 0;
#endif
  ...
  printf("\nLow-precision GEMM (%s): %.2f TFLOPS (avg %.2f ms)\n", low_name, rlow.tflops, rlow.ms);

  // KV bandwidth
  float gbps=0; bench_kv(5.0,&gbps);
  ...
  if (p.major>=10 && low_name!= (const char*)"fp4_native"){
    printf("Note: upgrade CUDA/cuBLASLt and wire NVFP4 to enable native FP4.\n");
  }
#endif
  return 0;
}
